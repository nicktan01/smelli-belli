...
class Cart(models.Model):
    ...
    user = models.IntegerField(
        null=True
    )  # changed this to match the changes from the quiz models. check
    # ./views.py to see how the user information was grabbed out of the
    # token
    created = models.DateField(auto_now_add=True)
...
class WishList(models.Model):
    user = models.IntegerField(
        null=True
    )  # same as line 35 on user prop for cart
    product = models.ForeignKey(
